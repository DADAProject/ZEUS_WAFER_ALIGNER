...
public struct IniValue
{
    ...
    public bool TryConvertShort(out short result)
    {
        ...
        if (TryParseShort(Value.Trim(), out result))
        {
            return true;
        }
        return false;
...
public class IniSection : IEnumerable<KeyValuePair<string, IniValue>>, IDictionary<string, IniValue>
{
    ...
    public IniSection()
        : this(TIniUnit2.DefaultComparer)
    {
    }
    ...
    public IniSection(Dictionary<string, IniValue> values)
        : this(values, TIniUnit2.DefaultComparer)
    {
    }
...
/* Note:                                                                   */
/***************************************************************************/
public class TIniUnit2 : IEnumerable<KeyValuePair<string, IniSection>>, IDictionary<string, IniSection>
{
    ...
    public TIniUnit2(IEqualityComparer<string> stringComparer)
    {
        StringComparer = stringComparer;
        sections = new Dictionary<string, IniSection>(StringComparer);
    }

    public void Saveini(string path, FileMode mode = FileMode.Create)
    {
    ...
    //------------------------------------------------------------------------
    public bool Loadini(string path, bool ordered = false)
    {
        if (!FNC.FileExists(path))
        {
            MessageBox.Show($"File Path Error! [{path}]");
            return false;
        }
